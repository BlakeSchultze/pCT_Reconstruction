...
__Code and Data:__   
______________
**(1)** Each pCT user has a subdirectory `/local/pCT_code/Reconstruction/<username>` for their code and users also have access to the release versions of each repository in `/local/pCT_code/git/<account>/<repo>`.  A copy of these can be cloned to a user's code directory using the bash function `add_rcode_repo` (also available in `pCT_Tools`).  Make sure to enter `git checkout release_development` to switch to this branch when cloning this repository.    
...
**(3)** Copy the input data to the compute node's local solid state drive, either to `/local/pCT_data/organized_data` if it is organized according to the standardized naming/organizational scheme or `/local/pCT_data/user_data/<username>` if  it is unorganized. The bash function `organize_data` available in the `pCT_Tools` repository can be used to organize data.  
**(4)** If the output data isn't going to be organized, it should be written to the user's `/local/pCT_data//user_data/<username>` subdirectory.    
