...
FILTER_TYPES FBP_FILTER			= SHEPP_LOGAN;		  					// Specifies which of the defined filters will be used in FBP
X_0_TYPES X_0					= HYBRID;								// Specify which of the HULL_TYPES to use in this run's MLP calculations
...
/*--------------------------------------------------------------------- Preprocessing option parameters -------------------------------------------------------------------*/
/*-------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
bool IMPORT_PREPROCESSED_DATA	= true;									// [T/F] Import preprocessed data previously generated, i.e. A/x0/b/hull/MLP), (T) or generate it (F) 
bool PERFORM_RECONSTRUCTION		= true;									// [T/F] Perform reconstruction (T) or not (F)
bool PREPROCESS_OVERWRITE_OK	= false;								// [T/F] Allow preprocessing data to be overwritten (T) or not (F)
bool RECON_OVERWRITE_OK 		= false;								// [T/F] Allow reconstruction data to be overwritten (T) or not (F)
