...
do
	...
	do
		...
		then
			...
			path_out="${relative_path}/Organized/${1}/Experimental/${run_date}/Run${run_num_dir}/Input"
			mkdir -p "${path_out}"
			ln -s "$full_path/${1}${run_num_dir}_$angle_str.dat.root.reco.root.bin" "${path_out}/raw_${angle_str}.bin"
		fi
	done
done
